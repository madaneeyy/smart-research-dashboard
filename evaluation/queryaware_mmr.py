...
A: Hybrid RRF -> relevance filter -> current metadata-aware MMR (lambda=0.7)
B: Hybrid RRF -> relevance filter -> query-aware combined-score ranking (NO MMR)
E: Hybrid RRF -> relevance filter -> query-aware combined-score MMR (lambda=0.7)
...
def rerank_query_aware_mmr(
    ...
    """
    MMR where the relevance term is the full query-aware relevance score.

    IMPORTANT:
      - Relevance is NOT just semantic similarity.
      - It is the same combined relevance used by the no-MMR experiment:
        semantic + lexical + metadata/query-intent signals.
      - Diversity is still measured in embedding space.
      - This isolates the question: does MMR help when MMR is driven by
        a better relevance signal?

    MMR:
        score(d) = lambda * query_aware_relevance(d)
                 - (1-lambda) * max_sim(d, selected)
    """
    ...
    working = [dict(c) for c in candidates]

    # Compute the query-aware relevance exactly once.
    for candidate in working:
        candidate["query_aware_score"] = query_aware_score(query, candidate)

    model = SimpleRetriever._get_model()

    texts = [str(c.get("content", "")) for c in working]
    ...
    while remaining and len(selected) < top_k:
        ...
        for i in remaining:
            relevance = float(working[i]["query_aware_score"])

            if not selected_indices:
                redundancy = 0.0
            else:
                sims = embeddings[selected_indices] @ embeddings[i]
                redundancy = float(np.max(sims)) if len(sims) else 0.0
        ...
        result = working[best_index]
        result["mmr_score"] = float(best_score)
        result["mmr_relevance"] = float(result["query_aware_score"])
        result["mmr_redundancy"] = float(best_redundancy)
...
def run_method(
    ...
    if method == "E_query_aware_mmr":
        return rerank_query_aware_mmr(
            query=query,
            candidates=[dict(c) for c in accepted],
            top_k=TOP_K,
            mmr_lambda=MMR_LAMBDA,
...
def report(evaluation: Dict[str, Any]) -> str:
    lines = [
        "=" * 110,
        "RANKING ABLATION: CURRENT MMR vs QUERY-AWARE vs COMPLEMENTARITY vs QUERY-AWARE MMR",
        "=" * 110,
        ...
        "  E uses the query-aware combined relevance score inside the MMR relevance term.",
        "",
        "METHOD SUMMARY",
        "-" * 110,
    ]
...
def main() -> None:
    print("=" * 110)
    print("RANKING ABLATION: CURRENT MMR vs QUERY-AWARE vs COMPLEMENTARITY vs QUERY-AWARE MMR")
    print("=" * 110)
    ...
    evaluation = {
        ...
        "configuration": {
            ...
            "experiment_E": "query-aware combined relevance -> embedding-space MMR; isolates whether MMR improves when driven by query-aware relevance",
        },
