...
class HybridRetriever:
    ...
    * Metadata should guide ranking, not overpower semantic relevance.
    ...
    def __init__(
        ...
        # Shared embedding provider used for query embeddings.
        # SimpleRetriever owns the persistent chunk-embedding cache.
    ...
    def _rerank(
        ...
        query_embedding = np.asarray(
            self._embedding_provider.embed(question),
            dtype=np.float32,
        ).reshape(-1)
        ...
        # SimpleRetriever.retrieve(). This avoids embedding the same
        # candidate chunks again.
